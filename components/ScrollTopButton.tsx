...
const ScrollTopButton = ({ divRef, setCurrentId, currentId, hadiths }) => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setCurrentId({
              id: parseInt(entry.target.id, 10),
              number: hadiths[entry.target.id - 1].number,
            });
          }
        });
      },
  ...
  const handleNext = () => {
    if (currentId.id < hadiths.length) {
      setCurrentId((prevId) => ({
        id: prevId.id + 1,
        number: hadiths[prevId.id - 1].number,
      }));
  ...
  return (
    <div
      className={
        showGoTop
          ? "flex items-center gap-2 fixed bottom-4 left-1/2 transform -translate-x-1/2 p-2 bg-royal-blue rounded-xl"
          : ""
      }
    >
      <p className="whitespace-nowrap pl-2 text-white text-sm">
        {currentId.number} of {hadiths[hadiths.length - 1].number}
      </p>
