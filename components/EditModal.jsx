...
function HadithItem({
  ...
  async function valueChanged(hadithId, chapterNumber) {
    let pushedValue = submittedValue.concat([
      { [hadithId]: value[chapterNumber] },
    ]);
    ...
    <div key={hadith._id} className="grid grid-cols-2 space-y-4 items-center">
      ...
      {value.length > 1 && (
        <div className="flex gap-2">
          <Input
            defaultValue={value[chapterNumber]}
            onChange={(e) => onChangeValue(e, chapterNumber)}
          />
          {submittedValue[chapterNumber] ? (
            <Button
              variant={"secondary"}
              onClick={() => valueChanged(hadith._id, chapterNumber)}
            >
              <CheckIcon size={12} color={"black"} />
            </Button>
          ) : (
            <Button onClick={() => valueChanged(hadith._id, chapterNumber)}>
          ...
          {submittedValue[chapterNumber] ? (
            <Button
              variant={"secondary"}
              onClick={() => removeValue(value, chapterNumber)}
            >
              <MinusIcon size={12} color={"black"} />
            </Button>
          ) : (
            <Button onClick={() => removeValue(value, chapterNumber)}>
...
function EditModal({ chapterId, hadiths }) {
  ...
  return (
    <Dialog>
      <DialogTrigger>
        <FileEditIcon size={16} color={"black"} />
      </DialogTrigger>
      <DialogContent
      ...
      >
        <DialogHeader>
          ...
          <div>
            {value.length === 0 && (
              <Textarea onChange={onChangeTransliteration} />
            )}
            {hadiths.map((hadith) => {
