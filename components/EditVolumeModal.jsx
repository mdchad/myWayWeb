...
function EditVolumeModal({ chapterId, hadiths}) {
  const [value, setValue] = useState({ ar: hadiths ? hadiths[0].volume_title.ar : null, ms: hadiths ? hadiths[0].volume_title.ms : null})
  ...
  return (
    <Dialog>
      <DialogTrigger><FileCogIcon size={16} color={'black'}/></DialogTrigger>
      <DialogContent className={"lg:max-w-screen-lg overflow-y-scroll max-h-screen"}>
        <DialogHeader>
          <DialogTitle>Are you absolutely sure?</DialogTitle>
          <Label>Volume Arabic</Label>
          <Input onChange={(e) => setValue({ ...value, ar: e.target.value})} className="font-arabic" dir="rtl" value={value.ar}/>
          <Label>Volume Malay</Label>
          <Input onChange={(e) => setValue({ ...value, ms: e.target.value})} value={value.ms}/>
        </DialogHeader>
        <DialogFooter>
          <DialogClose asChild>
