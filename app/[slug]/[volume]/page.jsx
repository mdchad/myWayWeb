...
async function getData(volume, slug) {
  ...
  if (bookId === "240360e4-50b4-47a9-9506-9850b0e3bfd7" && volume.id !== "2b1bc287-cdea-4e51-b5d3-f6fa0ce31235") { //check for muslim because they have different way of sort
    return await db.collection('Hadiths').find({ volume_id: volume.id, book_id: bookId }).sort({ _id: 1 }).toArray();
  }
  return await db.collection('Hadiths').find({ volume_id: volume.id, book_id: bookId }).sort({ number: 1 }).toArray();
}
...
export default async function Hadiths({ params }) {
  ...
  return (
    <HadithContainer hadiths={JSON.parse(JSON.stringify(hadiths))} volumes={JSON.parse(JSON.stringify(volume))} surahs={JSON.parse(JSON.stringify(surah))} />
  )
}